...
def apply_action_to_stone(action_tuple: tuple, stones: list, target_color_tuple: tuple, model_path: str = ""):
    """
    Rule-based 행동에 오차를 적용하여 해당 색상의 돌에 impulse를 적용합니다.
    model_path를 받아 AlggaGo1.0일 경우 '일반 공격'을 강제합니다.
    """
    ...
    # --- 핵심 수정: 모델 경로에 따라 행동 결정 ---
    if "AlggaGo1.0" in model_path:
        # AlggaGo1.0일 경우, '일반 공격'만 사용
        rule_action = get_regular_action(player_stones, opponent_stones)
        print("[Action Strategy] AlggaGo1.0 모드: 일반 공격(Regular Action) 사용")
    else:
        # 그 외 모델은 기존 로직 (틈새 공격 우선) 사용
        rule_action = get_split_shot_action(player_stones, opponent_stones)
        if rule_action is None:
            rule_action = get_regular_action(player_stones, opponent_stones)
            print("[Action Strategy] 틈새 공격 불가: 일반 공격(Regular Action)으로 전환")
        else:
            print("[Action Strategy] 틈새 공격(Split Shot) 사용")
    # --- 수정 끝 ---
...
def choose_ai():
    ...
    # [수정] 새로운 파일명 규칙(타임스텝 + 엔트로피)에 맞는 정규 표현식
