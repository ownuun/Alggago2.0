...
def run_gauntlet_training(model_to_train, model_name, initial_timesteps, current_threshold):
    """
    주어진 모델이 모델 C를 이길 때까지 훈련하는 예선전 함수. (최종 버전)
    """
    print("\n" + "="*50)
    print(f"🥊      특별 예선 시작: 모델 {model_name} vs 모델 C       🥊")
    print(f"   - 현재 틈새 공격 성공률 기준: {current_threshold:.2%}")
    print("="*50)
    ...
    # --- [✅ 최종 수정] 모든 경우의 수를 처리하는 모델 로드/생성 로직 ---
    if os.path.exists(GAUNTLET_SAVE_PATH):
        ...
        if model_to_train is None:
            # Case 1: 완전한 새 학습 시작 (main의 except 블록에서 호출)
            ...
            # Case 2: 재검증 시작 (main의 for 루프에서 기존 모델 전달)
        ...
        if win_rate > 0.5:
            ...
            return model_to_train, model_to_train.num_timesteps, current_threshold
    ...
    while True:
        ...
        print("\n[INFO] 모델 저장 및 예선 통과 조건 확인...")
        if reg_success > 0.5 and split_success > current_threshold:
            model_to_train.save(GAUNTLET_SAVE_PATH)
            print(f"   ✅ 성공률 기준(일반 >50%, 틈새 >{current_threshold:.2%}) 통과! 모델 저장.")
            new_threshold = min(0.8, current_threshold + 0.01)
            if new_threshold != current_threshold:
                print(f"   📈 다음 틈새 공격 성공률 기준이 {new_threshold:.2%}로 상향 조정됩니다.")
            current_threshold = new_threshold
        else:
            print(f"   ❌ 성공률 미달 (기준: 일반 >50%, 틈새 >{current_threshold:.2%}). 롤백합니다.")
    ...
    return model_to_train, current_total_timesteps, current_threshold
...
def main():
    ...
    try:
        ...
        # 초기 예선전은 A모델만 진행
        model_A, total_timesteps_so_far, split_shot_threshold = run_gauntlet_training(
            model_to_train=None, 
            model_name="A", 
            initial_timesteps=0,
            current_threshold=split_shot_threshold
        )
    ...
    for stage_idx in range(start_stage, MAX_STAGES):
        ...
        if model_to_requalify:
            trained_model, total_timesteps_so_far, split_shot_threshold = run_gauntlet_training(
